...
- verify-install.sh gains a Layer 0h section: ownership checks on `/etc/pi`, then behavioural probes through the real wrapper — `~/.ssh` and a runtime-created `.env` must fail with EPERM, `.envelope` and `/etc/hosts` must succeed, and `npm config get ignore-scripts` must agree inside and outside the sandbox. The extension is checked for presence, and an extension installed without `/etc/pi` is reported as the fail-closed state it is.
